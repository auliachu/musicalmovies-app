...
<?php
            ...
            $ambil= "SELECT faktur.id_jadwaltayang,faktur.total,musikal.nama_musikal,musikal.id_musikal,  COUNT(id_jadwaltayang) as jumlah, SUM(total) as totalx  FROM faktur, 
             musikal where faktur.id_musikal=musikal.id_musikal and faktur.tanggal_pesan BETWEEN '$tgl_awal' AND '$tgl_akhir' GROUP BY id_jadwaltayang";
...
<hr />
		...
		$sql = mysqli_query($konek, $ambil); // 
                    ...
                    if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
                        while($pecah = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
							...
							$no++;
								$total=$pecah['jumlah'];
								$hitungtotal=$pecah['totalx'];
	//$total=$pecah['total'];
	//$bayar=$total-($total*($diskon/100));
								@$totals += $total;	
								@$hitungtotalmusikal+=$hitungtotal;
							?>
		<tr>
		  <td><?php echo $no; ?></td>
		  ...
		  <td><?php echo number_format($hitungtotal); ?></td>
		</tr>
	...
	<?php
		...
		<td colspan="3" >Total </td>
			...
			<td><?php echo number_format($hitungtotalmusikal); ?> </td>
		</tr> <?php
